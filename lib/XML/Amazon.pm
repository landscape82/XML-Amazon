...
sub asin{
	...
	$ITEM->{artist} = $pl->{Items}->{Item}->{ItemAttributes}->{Artist};
	$ITEM->{releasedate} = $pl->{Items}->{Item}->{ItemAttributes}->{ReleaseDate};
	$ITEM->{origreleasedate} = $pl->{Items}->{Item}->{ItemAttributes}->{OriginalReleaseDate};
...
sub search{
	...
	if ($pl->{Items}->{Item}->[0]->{ASIN}){
		...
		for (my $i = 0; $pl->{Items}->{Item}->[$i]; $i++){
			...
			$new_item->{artist} = $pl->{Items}->{Item}->[$i]->{ItemAttributes}->{Artist};
			$new_item->{releasedate} = $pl->{Items}->{Item}->[$i]->{ItemAttributes}->{ReleaseDate};
			$new_item->{origreleasedate} = $pl->{Items}->{Item}->[$i]->{ItemAttributes}->{OriginalReleaseDate};
...
=head2 $XML_Amazon_Item->artist
=head2 $XML_Amazon_Item->releasedate
=head2 $XML_Amazon_Item->origreleasedate
